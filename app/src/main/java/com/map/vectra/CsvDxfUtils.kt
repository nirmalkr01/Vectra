...
object CsvDxfUtils {
    ...
    fun mapToPoints(rawRows: List<List<String>>, xIndex: Int, yIndex: Int): List<PointData> {
        ...
        rawRows.forEach { row ->
            if (row.size > xIndex && row.size > yIndex) {
                val x = row[xIndex].toFloatOrNull()
                val y = row[yIndex].toFloatOrNull()
                if (x != null && y != null) {
    ...
    fun calculatePolygonArea(points: List<PointData>): Float {
        var area = 0.0f
        val n = points.size
        for (i in 0 until n) {
            val j = (i + 1) % n
    ...
    fun distance(p1: PointData, p2: PointData): Float {
    ...
    fun distanceToSegment(p: PointData, v: PointData, w: PointData): Float {
        val l2 = (v.x - w.x).pow(2) + (v.y - w.y).pow(2)
        if (l2 == 0f) return distance(p, v)
